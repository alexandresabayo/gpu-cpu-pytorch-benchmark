...
def run_training_experiment(model: nn.Module, loaders: Dict[str, DataLoader],
    ...
    Opens a "Training on CPU"/"Training on GPU" child step under the
    currently active step for the training loop itself. That child step
    closes (and collapses to one permanent line) before the metrics table
    is printed, so the metrics table survives as its own permanent block
    rather than getting collapsed away with the training detail.
    """
    step = current()
    device_label = "Training on CPU" if device.type == "cpu" else "Training on GPU"

    with step.child(device_label):
    ...
    # The child step has now closed and collapsed to its one permanent
    # line. The header + metrics table below are printed as their own
    # permanent blocks, so they stay visible regardless of that collapse.
...
def run_experiment(experiment_name: str, model_cpu: nn.Module, model_gpu: nn.Module,
    ...
    Expects a step to already be open (e.g. main.py's per-experiment step,
    titled with the experiment name) — this function logs into it via
    richlog.current() and opens the CPU/GPU child steps, but does not open
    the experiment-level step itself.
    """
    step = current()
